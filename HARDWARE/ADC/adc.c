...
{    
		...
		RCC_APB2PeriphClockCmd(RCC_APB2Periph_ADC|RCC_APB2Periph_ADC1|RCC_APB2Periph_ADC2|RCC_APB2Periph_ADC3, ENABLE); //使能ADC1时钟
		...
		//先初始化ADC1通道5 IO口
		GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0|GPIO_Pin_3|GPIO_Pin_4;     	  //PC3 通道
		GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AN;  		//模拟输入
		GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_NOPULL;  //不带上下拉
		GPIO_Init(GPIOC, &GPIO_InitStructure);  					//初始化  
	 
		//先初始化ADC1通道10 IO口
		GPIO_InitStructure.GPIO_Pin = GPIO_Pin_3;     	  //PC3 通道
		GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AN;  		//模拟输入
		GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_NOPULL;  //不带上下拉
		GPIO_Init(GPIOC, &GPIO_InitStructure);  					//初始化  
...
//获得ADC值
//ch: @ref ADC_channels
//通道值 0~16取值范围为：ADC_Channel_0~ADC_ Channel_16
//返回值:转换结果
u16 Get_Adc1(u8 ch)
{
